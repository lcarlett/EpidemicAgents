...
class Desease(object):
    Sane, Infected, Recovered = range(3)
    def __init__(self, infectivity, recoverability, immunity_rate, infectionRadius = 1):
        self._transmission_rate = infectivity
        self._recovery_rate = recoverability
        self.infectionRadius = lambda: infectionRadius
        self.immunity_rate = lambda: immunity_rate
        self._infected = 0
        self._removed = 0
    ...
    def apply_to(self, agent, surrounding):
        ...
        if agent.isInfected() and random() < self._recovery_rate:
            agent.setSane()
            self._infected -= 1
            if self.immunity_rate():
                self._removed += 1
                agent.setRecovered()
        elif not agent.isInfected() and binomial(self._transmission_rate*agent.vulnerability(), inf_neigh):
            agent.setInfected()
            self._infected += 1 
        elif agent.isRecovered() and random() < 1-self.immunity_rate():
            self._removed -= 1
            agent.setRecovered(False)
